...
class TrayApp:
    ...
    def _show_settings(self) -> None:
        ...
        # Enable checkbox (bridge-level flag)
        ...
        # Row 2: Browse | Activate/Deactivate toggle | status badge
        pbtn_row = tk.Frame(pf, bg=_C_PANEL)
        pbtn_row.grid(row=2, column=0, columnspan=2, sticky="ew",
                      padx=(10, 10), pady=(4, 4))

        # Row 3: hint about automatic backup
        tk.Label(
            pf,
            text="  \u2139 " + T("proxy_bak_hint"),
            bg=_C_PANEL, fg=_C_FG_DIM, font=("Segoe UI", 7), anchor="w",
        ...
        def _refresh_proxy_status() -> None:
            from bridge.hub import Hub
            s = Hub.proxy_status()
            if s == "active":
                _proxy_status_lbl.config(
                    text=T("proxy_status_active"),
                    fg="#6f6", bg="#1a3d1a",
                )
                _proxy_toggle_btn.config(
                    text=T("proxy_btn_deactivate"),
                    bg="#5c1a1a", activebackground="#7a2222",
                )
            else:
                _proxy_status_lbl.config(
                    text=T("proxy_status_inactive"),
                    fg="#fa0", bg="#3d2a00",
                )
                _proxy_toggle_btn.config(
                    text=T("proxy_btn_activate"),
                    bg=_C_ACCENT, activebackground="#005a9e",
                )
        ...
        def _toggle_proxy() -> None:
            ...
            if s == "active":
                ...
                if ok:
                    _refresh_proxy_status()
                    msg_key = (
                        "proxy_deactivated_restored"
                        if detail == "restored"
                        else "proxy_deactivated_removed"
                    )
                    messagebox.showinfo(T("saved_title"), T(msg_key), parent=win)
                else:
                    messagebox.showerror(
                        T("err_title"),
                        T("settings_proxy_setup_err", err=detail),
                        parent=win,
                    )
            else:
                ...
                if not path:
                    messagebox.showerror(
                        T("err_title"), T("settings_proxy_no_path"), parent=win
                    )
                    return
                ...
                if ok:
                    ...
                    messagebox.showinfo(
                        T("saved_title"),
                        T("proxy_activated_msg", path=result),
                        parent=win,
                    )
                else:
                    messagebox.showerror(
        ...
        # ── widgets (functions already defined above) ───────────────────
        tk.Button(
        ...
        _proxy_toggle_btn = tk.Button(
            pbtn_row, text=T("proxy_btn_activate"), command=_toggle_proxy,
            bg=_C_ACCENT, fg="white",
            activebackground="#005a9e", activeforeground="white",
            relief="flat", cursor="hand2",
            font=("Segoe UI", 9, "bold"), padx=12, pady=4,
        )
        _proxy_toggle_btn.pack(side="left", padx=(0, 8))

        _proxy_status_lbl = tk.Label(
            pbtn_row, text="", bg="#333", fg="#888",
            font=("Segoe UI", 8), padx=8, pady=3,
            relief="flat",
        )
        _proxy_status_lbl.pack(side="left")

        _refresh_proxy_status()  # set initial state
